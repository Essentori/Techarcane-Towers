...
public class PlayerFlyController : MonoBehaviour
{
    ...
    {
        _player.State.SetState(PlayerState.Flying, true);
    ...
    {
        _player.State.SetState(PlayerState.Flying, false);
        _flyVerticalVelocity = Vector3.zero;
    }
    ...
    private void HandleFlyVerticalMovement()
    {
        ...
        if (_player.InputHandler.JumpIsPressed) verticalDirection += 1f;
        if (_player.InputHandler.CrouchIsPressed) verticalDirection -= 1f;
